...
export default function SearchResultsPage() {
  ...
  return (
    <div className="space-y-8 py-4">
      ...
      <div className="glass-panel p-6 sm:p-7 rounded-3xl space-y-6 shadow-xl relative overflow-hidden bg-white/90">
        
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-200 text-[11px] font-extrabold uppercase tracking-wider text-indigo-700">
          ...
          {job && job.status === "completed" && (
            <div className="flex items-center gap-3">
              <a
                ...
                download
                className="px-4 py-2.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all shadow-sm active:scale-95"
              >
              ...
              <a
                ...
                download
                className="px-4 py-2.5 bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 rounded-2xl text-xs font-bold flex items-center gap-2 transition-all shadow-sm active:scale-95"
              >
                <Download className="w-4 h-4 text-sky-600" /> Export JSON
              </a>
            </div>
          )}
        </div>
          ...
          <div className="space-y-4 pt-2 border-t border-slate-100">
            <div className="flex items-center justify-between text-xs font-bold text-slate-700">
              <span className="capitalize flex items-center gap-2 text-sm">
                ...
                Pipeline Status: <strong className="text-indigo-600 uppercase tracking-wider">{job.status}</strong>
              </span>
            ...
            <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden border border-slate-200 p-0.5">
              <div
                className="h-full bg-gradient-to-r from-indigo-600 via-purple-600 to-sky-500 transition-all duration-700 rounded-full shadow-md shadow-indigo-500/20"
            ...
            <div className="flex items-center gap-6 text-xs text-slate-600 font-semibold pt-1">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span>Qualified Matches: <strong className="text-emerald-700 font-extrabold">{job.qualified}</strong></span>
              </div>
            </div>
          </div>
        )}
      ...
      {job?.structured_plan && (
        <div className="glass-panel p-4 sm:p-5 rounded-2xl text-xs space-y-2 bg-white">
          ...
          <div className="flex items-center gap-6 flex-wrap text-slate-800 font-semibold text-xs">
            <div>Target Entity: <span className="font-extrabold text-indigo-700 uppercase px-2.5 py-0.5 rounded-md bg-indigo-50 border border-indigo-100">{job.structured_plan.entity_type}</span></div>
