    ...
    <div className="space-y-8 max-w-4xl mx-auto py-4">
      ...
      <div className="space-y-2">
        ...
        <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight flex items-center gap-3">
          <Key className="w-8 h-8 text-indigo-600" /> API Keys & Access Tokens
        </h1>
      ...
      {newKey && (
        <div className="glass-panel p-5 rounded-3xl border-2 border-indigo-500 bg-indigo-50/90 space-y-3 shadow-lg animate-pulse">
          ...
          <div className="flex items-center gap-3 bg-white p-3.5 rounded-2xl border border-indigo-200 shadow-sm">
            <code className="text-indigo-900 font-mono text-sm break-all flex-1 select-all">{newKey}</code>
            <button
              onClick={() => copyToClipboard(newKey)}
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shrink-0 shadow-md shadow-indigo-600/20 active:scale-95 transition-all"
            >
      ...
      <form onSubmit={handleCreate} className="glass-panel p-6 rounded-3xl border border-slate-200 space-y-4 shadow-md bg-white">
        <h2 className="text-sm font-bold uppercase tracking-wider text-slate-700 flex items-center gap-2">
          <Plus className="w-4 h-4 text-indigo-600" /> Generate New Integration API Key
        </h2>
        <div className="flex items-center gap-3 flex-col sm:flex-row">
          ...
          <button
            ...
            className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-indigo-600 to-sky-600 hover:from-indigo-700 hover:to-sky-700 text-white font-bold text-sm rounded-2xl flex items-center justify-center gap-2 transition-all disabled:opacity-40 shadow-lg shadow-indigo-600/20 shrink-0 active:scale-95"
          >
            {loading ? (
              ...
              {keys.length === 0 ? (
                ...
                keys.map((k) => (
                  <tr key={k.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-4 font-mono text-indigo-600">{k.key_prefix}...</td>
                    <td className="px-5 py-4">
                      <span className="px-2.5 py-1 rounded-lg font-bold text-[10px] uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200">
                        Active
                      </span>
                    </td>
