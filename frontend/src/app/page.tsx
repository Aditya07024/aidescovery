  ...
  return (
    <div className="space-y-12 sm:space-y-16 py-6 sm:py-10">
      ...
      <div className="text-center space-y-5 max-w-4xl mx-auto px-4 relative">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[32rem] h-[32rem] bg-emerald-500/10 rounded-full blur-3xl pointer-events-none -z-10 animate-glow"></div>

        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-panel text-xs font-bold text-slate-800 border border-emerald-200 bg-white/90 shadow-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4.5 pt-8">
        {[
          ...
          return (
            <div
              key={idx}
              className="glass-panel glass-panel-hover p-6 rounded-3xl border border-slate-200 bg-white/90 space-y-3 relative group shadow-sm"
            >
