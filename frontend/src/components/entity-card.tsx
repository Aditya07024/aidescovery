...
export function EntityCard({ item }: EntityCardProps) {
  ...
  return (
    <div className="glass-panel glass-panel-hover rounded-3xl p-5.5 space-y-4 relative flex flex-col justify-between border border-slate-200 bg-white/90 group shadow-md">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-extrabold uppercase tracking-wider text-indigo-700 px-2.5 py-0.5 rounded-lg bg-indigo-50 border border-indigo-200 flex items-center gap-1">
                <EntityTypeIcon className="w-3 h-3 text-indigo-600" />
                {item.entity_type}
              </span>

              {item.is_qualified ? (
                <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-700 px-2 py-0.5 rounded-lg bg-emerald-50 border border-emerald-200 flex items-center gap-1">
      ...
      <div className="space-y-3 pt-2">
        ...
        {item.qualification_reasons && item.qualification_reasons.length > 0 && (
          <div className="bg-slate-50/80 rounded-2xl p-3 border border-slate-200 space-y-1.5">
            <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-600 flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5 text-indigo-600" /> AI Fact Qualification:
            </span>
        ...
        <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600 flex-wrap gap-2">
          ...
          <Link
            ...
            className="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center gap-1 transition-all ml-auto group-hover:translate-x-0.5"
          >
