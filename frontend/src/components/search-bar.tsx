  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-7">
      ...
      <form onSubmit={handleSubmit} className="relative group">
        ...
        {/* Glow backdrop */}
        <div className="absolute -inset-1 bg-gradient-to-r from-emerald-600 via-teal-600 to-slate-800 rounded-3xl blur-xl opacity-20 group-hover:opacity-40 transition duration-700"></div>

        <div className="relative glass-panel rounded-3xl p-3.5 sm:p-4 space-y-3.5 shadow-xl border border-slate-200 bg-white/95">
          ...
          <div className="flex items-center gap-3 px-3 py-1">
            ...
            <button
              ...
              className="px-6 py-3.5 bg-slate-900 hover:bg-emerald-700 text-white text-sm font-bold rounded-2xl flex items-center gap-2 shadow-lg shadow-slate-900/20 disabled:opacity-40 transition-all shrink-0 active:scale-95"
            >
              {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {PRESET_CATEGORIES.map((preset, idx) => (
            <button
              ...
              className="text-left p-4 glass-panel rounded-2xl text-slate-700 hover:text-slate-900 border border-slate-200 hover:border-emerald-300 transition-all flex items-start justify-between gap-3 group glass-panel-hover bg-white/90"
            >
              <div className="space-y-1.5">
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-800 px-2.5 py-0.5 rounded-md bg-emerald-50 border border-emerald-200">
                    {preset.category}
                  </span>
                ...
                <p className="text-xs text-slate-800 group-hover:text-emerald-950 transition-colors leading-relaxed font-medium">
                  "{preset.query}"
                </p>
              </div>

              <div className="w-7 h-7 rounded-xl bg-slate-100 group-hover:bg-emerald-50 border border-slate-200 group-hover:border-emerald-200 flex items-center justify-center shrink-0 text-slate-400 group-hover:text-emerald-700 transition-all mt-0.5">
